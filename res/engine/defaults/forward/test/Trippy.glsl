...
void main()
{
    ...
    vec3 RF = refract(I, normalize(tNormals), ratio);
    vec3 RL = reflect(I, normalize(tNormals));
    vec3 R  = mix(RF, RL, ((sin(GE_TIME)/2)+0.5));
    /*colour = mix(vec4(texture(SKYBOX, R).rgb, 1.0),
                 vec4(1),
                 ((sin(GE_TIME)/2)+0.5));*/
